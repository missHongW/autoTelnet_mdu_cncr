...
from utils import handle_yaml, handle_path, handle_excel,excelTest
...
class TestDeviceInfo:
    ...
    def setup_class(self):
        ...
        print(f"议连接信息:{self.link_info}")
        print("link内容：" + str(self.link_info))
        ...
        print(f"发送的指令，上电参数：{message}")
        self.pb.send_serial(port='COM10', cycle_time=1)
        # 发起socket连接【udp 产测】
    ...
    def test_device_info(self, case_list_command):
        ...
        print(f" 测试用例集：{case_list_command}")
        lb.logger.info(
        ...
        print(f" 测试用例名称：{case_list_command['case_name']},测试用例类型：{case_list_command['case_type']}, 测试用例步骤：{type(case_list_command['case_step'])},测试用例步骤：{case_list_command['case_step']} ")

        device_expect = case_list_command['expect_result'].replace('\n', '')
        lb.logger.info(f"用例期望结果：{type(device_expect)}， {device_expect} ")
        print(f"用例期望结果：{type(device_expect)}， {device_expect} ")
